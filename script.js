...
/* ── AUTO LOAD HELPER ── */
  ...
  */
  ...
  function switchTab(tab, fromHash) {
    ...
    // Reels tab থেকে অন্য tab এ গেলে — reels feed silently বন্ধ করো (modal-close anim/back ছাড়াই)
    if (currentTab === 'reels' && tab !== 'reels' && typeof _closeReelsTabSilently === 'function') {
      _closeReelsTabSilently();
    }
    ...
    if (!fromHash) {
      var _newHash = '#' + (tab === 'podcast' ? 'listen' : tab === 'watch' ? 'home' : tab);
    ...
    var tReels = document.getElementById('tabReels');
    if (tReels) tReels.classList.toggle('active', tab === 'reels');
    ...
    var dr = document.getElementById('dsbTabReels');
    ...
    if (dr) dr.classList.toggle('active', tab === 'reels');
    ...
    // Reels — স্বাভাবিক tab হিসেবে feed খোলো (fullscreen overlay নয়, tabBar visible থাকবে)
    if (tab === 'reels' && typeof _openReelsTabPage === 'function') _openReelsTabPage();
  }
  /* ── THEME (Day/Night) ── ডিফল্ট Day; toggle সাইডবারের Appearance রো থেকে ── */
  function applyTheme() {
    ...
    var bgColor = theme === 'dark' ? '#0f0f13' : '#f7f7f5';
    ...
    if (window.AndroidBridge && window.AndroidBridge.setStatusBarColor) {
      window.AndroidBridge.setStatusBarColor(bgColor);
    }
  ...
  function setTheme(theme) {
    ...
    applyTheme();
    if (typeof saveTheme === 'function') saveTheme(theme);
  }
  window.setTheme = setTheme;
  function updateThemePillUI(theme) {
    var lightBtn = document.getElementById('themePillLight');
    var darkBtn  = document.getElementById('themePillDark');
  ...
  function applyAccent() {
    ...
    document.documentElement.style.setProperty('--fp-grad1', '#200e0d');
    document.documentElement.style.setProperty('--fp-grad2', '#301311');
    document.documentElement.style.setProperty('--fp-grad3', '#180908');
  }
  /* init accent */
  applyAccent();
  /* init theme — ডিফল্ট Day, localStorage/Firebase prefs অনুযায়ী override হয় */
  applyTheme();
  ...
  function renderStations(query = '') {
    ...
    _cachedStationsHtml = list.slice(0, 9).map(function(s, i) {
...
if ('serviceWorker' in navigator) {
  ...
  // Theme & Accent
  function saveTheme(theme) { fbSet('prefs/theme', theme); }
    ...
    window._fbGet(window._fbRef(window._fbDb, 'users/' + user.uid + '/prefs'))
      .then(function(snap) {
        ...
        if (prefs.theme && (prefs.theme === 'dark' || prefs.theme === 'light')) {
          try { localStorage.setItem('theme', prefs.theme); } catch(e) {}
          applyTheme();
        }
  ...
  function makeWatchVideoCard(v, i) {
    ...
    return '<div class="watch-video-card" style="animation-delay:' + (i * 0.06) + 's" data-wkey="' + key + '">'
      ...
      + '<div class="watch-video-play-btn"><i class="fa-solid fa-play"></i></div>'
      + '</div>'
  ...
  // Watch tab PPP row — Listen tab-এর podcast grid card (pepBuildRow) reuse করে,
  // যাতে exact same square-thumb grid card style থাকে। Tap করলে pcEpPillPlay() → mini player-এ audio play।
  function makeFyPodcastCard(ep, i) {
    var key = ep._key || '';
    return pepBuildRow(ep, i, "pcEpPillPlay('" + key + "')", true);
  }
  ...
  var _modalTitleIds = {
    ...
    browseCatsAll: 'browseCatsAllTitle',
    allWatchArtists: 'allWatchArtistsTitle',
    watchArtistVid: 'watchArtistVidTitle',
    sports: 'sportsModalTitle'
  };
  ...
  /* ══════════════════════════════════════
     HERO BANNER SECTION (Featured videos)
  ══════════════════════════════════════ */
  var _heroBannerTimer = null;
  var _heroBannerIdx = 0;
  var _heroBannerVids = [];
  function renderHeroBannerSection() {
    var section = document.getElementById('heroBannerSection');
    ...
    if (!featured.length) { section.style.display = 'none'; track.innerHTML = ''; dotsEl.innerHTML = ''; return; }
    section.style.display = '';
    _heroBannerVids = featured;
    _heroBannerIdx = 0;
    ...
    dotsEl.innerHTML = featured.map(function(v, i) {
      return '<div class="hero-banner-dot' + (i === 0 ? ' active' : '') + '" data-hdot="' + i + '"></div>';
    }).join('');
    track.querySelectorAll('.hero-banner-slide').forEach(function(slide) {
      slide.onclick = function() {
        var key = slide.dataset.hkey;
        if (key && typeof openWatchVideo === 'function') openWatchVideo(key);
      };
    });
    track.onscroll = function() {
      var idx = Math.round(track.scrollLeft / track.clientWidth);
      if (idx !== _heroBannerIdx && idx >= 0 && idx < _heroBannerVids.length) {
        _heroBannerIdx = idx;
        _updateHeroBannerDots();
      }
    };
    _startHeroBannerAutoplay();
  }
  function _updateHeroBannerDots() {
    var dotsEl = document.getElementById('heroBannerDots');
    if (!dotsEl) return;
    dotsEl.querySelectorAll('.hero-banner-dot').forEach(function(dot, i) {
      dot.classList.toggle('active', i === _heroBannerIdx);
    });
  }
  function _startHeroBannerAutoplay() {
    if (_heroBannerTimer) clearInterval(_heroBannerTimer);
    if (_heroBannerVids.length < 2) return;
    _heroBannerTimer = setInterval(function() {
      var track = document.getElementById('heroBannerTrack');
      if (!track || !track.clientWidth) return;
      _heroBannerIdx = (_heroBannerIdx + 1) % _heroBannerVids.length;
      track.scrollTo({ left: _heroBannerIdx * track.clientWidth, behavior: 'smooth' });
      _updateHeroBannerDots();
    }, 4500);
  }
  /* ══════════════════════════════════════
  ...
  // cat id 1-11  → category card For You তে দেখাবে না
  // cat id 12+   → video card    For You তে দেখাবে না
  ...
  // ── For You — PPP row এর জন্য podcast episode pool cycling (cat pattern অনুসরণ করে) ──
  var _fyPodShuffled = [];
  var _fyPodUsedIdx  = 0;

  function _fyShufflePods() {
    var eps = (typeof _podcastEps !== 'undefined' && _podcastEps.length)
      ? _podcastEps.filter(function(e){ return e && e._key && e.url; })
      : [];
    eps = eps.slice();
    for (var i = eps.length - 1; i > 0; i--) {
      var j = Math.floor(Math.random() * (i + 1));
      var tmp = eps[i]; eps[i] = eps[j]; eps[j] = tmp;
    }
    return eps;
  }

  function _fyNextPod() {
    if (!_fyPodShuffled.length || _fyPodUsedIdx >= _fyPodShuffled.length) {
      _fyPodShuffled = _fyShufflePods();
      _fyPodUsedIdx  = 0;
    }
    if (!_fyPodShuffled.length) return null;
    return _fyPodShuffled[_fyPodUsedIdx++];
  }

  function _fyNextPodTriple() {
    var eps = [];
    for (var i = 0; i < 3; i++) {
      var p = _fyNextPod();
      if (!p) break;
    ...
    return eps; // মোট podcast 3 এর কম থাকলে <3 length ফেরত আসবে
  }
   ...
   *   'PPP' — 3 podcast ep            → vids consumed: 0 (allowPodcast হলে)
   *
   * weight (forceCat=false): VVV=4, CV=2, CC=1, PPP=2 (allowPodcast হলে)
   * weight (forceCat=true):  CV=3,  CC=2  (VVV/PPP excluded)
   */
  function _fyPickRowType(forceCat, vidsLeft, allowPodcast) {
    if (forceCat) {
    ...
    var r = Math.random();
    if (allowPodcast) {
      // VVV 4/9, CV 2/9, CC 1/9, PPP 2/9
      if (vidsLeft >= 3 && r < 0.4444) return 'VVV';
      if (vidsLeft >= 1 && r < 0.6667) return 'CV';
      if (r < 0.7778) return 'CC';
      return 'PPP';
    }
  ...
  function _fyBuildMixedRows(vidPool, totalVidsShownBefore) {
    ...
    var rowNo  = 0;
    var lastCat = null;
    var allowPodcast = (typeof _podcastEps !== 'undefined') &&
      _podcastEps.filter(function(e){ return e && e._key && e.url; }).length >= 3;
    ...
    while (vIdx < vidPool.length) {
      ...
      var type      = _fyPickRowType(forceCat, vidsLeft, allowPodcast);

      // edge: not enough videos for VVV → downgrade
      if (type === 'VVV' && vidsLeft < 3) type = vidsLeft >= 1 ? 'CV' : 'CC';

      var row = { type: type, vids: [], cats: [], pods: [] };

      if (type === 'PPP') {
        var pods = _fyNextPodTriple();
        if (pods.length < 3) {
          // পর্যাপ্ত podcast নেই → video row-এ downgrade করো
          type = vidsLeft >= 3 ? 'VVV' : (vidsLeft >= 1 ? 'CV' : 'CC');
          row.type = type;
        } else {
          row.pods = pods;
        }
      ...
      if (type === 'PPP') {
        // pods সেট হয়ে গেছে — video pool থেকে কিছু consume হয় না
      } else if (type === 'VVV') {
      ...
      // safety: CC/PPP row consumes 0 videos — cap rows to avoid infinite loop
      if ((type === 'CC' || type === 'PPP') && vIdx === 0 && rowNo > 4) break;
    }
    return rows;
  }
  ...
  function _fyRenderRows(rows, grid, vidOffset) {
    rows.forEach(function(row) {
      ...
      } else if (row.type === 'PPP') {
        wrapper.className = 'fy-row-ppp';
        var ppCount = 0;
        row.pods.forEach(function(ep, pi) {
          var tmp = document.createElement('div');
          tmp.innerHTML = makeFyPodcastCard(ep, pi);
          var card = tmp.firstElementChild;
          if (card) { wrapper.appendChild(card); ppCount++; }
        });
        if (ppCount === 0) return; // pods না থাকলে এই row skip
  ...
  // mp4/webm এর জন্য সরাসরি src, m3u8 (HLS) এর জন্য hls.js attach করে
  function _reelSetVideoSrc(video, src) {
    if (!video || !src) return;
  ...
  function _reelClearVideoSrc(video) {
    if (!video) return;
    if (video._hls) { try { video._hls.destroy(); } catch(e) {} video._hls = null; }
    video.pause();
  ...
  var _reelsOpenedAsTab = false;
  var _reelsViewsPath   = 'reels';
  function openReelsModal(startIdx, fromPopstate, fromReelsAll, customReels, openedAsTab, viewsPath) {
    var reels = customReels || window._reelsData || [];
    if (!reels.length) return;
    _reelsOpenedAsTab = !!openedAsTab;
    _reelsViewsPath   = viewsPath || 'reels';
    ...
    document.body.classList.toggle('reels-as-tab', !!openedAsTab);
    ...
    if (firstVid) { _reelSetVideoSrc(firstVid, _reelsShuffled[0].url || ''); firstVid.play().catch(function(){}); }
    modal.classList.add('open');
    document.body.style.overflow = 'hidden';
    if (!fromPopstate) {
    ...
    function _appendNext(currentIdx) {
      ...
      for (var i = 1; i <= PRELOAD_AHEAD; i++) {
        ...
        if (video && src && !video.dataset.srcSet) {
          (function(v, s, delay) {
            setTimeout(function() {
              if (!v.dataset.srcSet) {
                v.preload = 'auto';
                _reelSetVideoSrc(v, s);
                v.dataset.srcSet = '1';
                v.load && v.load();
              }
            }, delay);
          ...
          _playDebounce = setTimeout(function() {
            ...
            track.querySelectorAll('.reel-slide').forEach(function(s) {
              if (s !== best) {
                ...
                if (v && sIdx < curIdx - 1) { _reelClearVideoSrc(v); delete v.dataset.srcSet; }
              }
            });
  ...
  function openReelsModalByKey(key, fromPopstate, fromReelsAll) {
    ...
    var track = document.getElementById('reelsModalTrack');
    if (track) track.querySelectorAll('video').forEach(function(v) { _reelClearVideoSrc(v); delete v.dataset.srcSet; });
  ...
  // ── Reels ট্যাব — সাধারণ tab হিসেবে খোলে, switchTab() থেকে call হয় ──
  function _openReelsTabPage() {
    var modal = document.getElementById('reelsModal');
    // আগে থেকেই reels tab খোলা থাকলে আবার নতুন random reel দিয়ে rebuild করার দরকার নেই
    if (modal && modal.classList.contains('open') && _reelsOpenedAsTab) return;
    var pool = window._reelsData || [];
    if (!pool.length) return;
    var randomStart = Math.floor(Math.random() * pool.length);
    // fromPopstate=true পাঠানো হচ্ছে যাতে openReelsModal নিজে আলাদা history push না করে —
    // switchTab() ইতিমধ্যে '#reels' hash push করে ফেলেছে
    openReelsModal(randomStart, true, false, pool, true, 'reels');
  }
  window._openReelsTabPage = _openReelsTabPage;
  // পুরনো নাম রাখা হলো backward-compatibility এর জন্য
  function openWatchReelsTab() { switchTab('reels'); }
  window.openWatchReelsTab = openWatchReelsTab;
  // Reels tab থেকে অন্য tab এ swipe/click করলে — কোনো close-animation বা history.back() ছাড়াই feed বন্ধ করো
  function _closeReelsTabSilently() {
    var modal = document.getElementById('reelsModal');
    var track = document.getElementById('reelsModalTrack');
    if (!modal) return;
    if (track) track.querySelectorAll('video').forEach(function(v) { _reelClearVideoSrc(v); delete v.dataset.srcSet; });
    modal.classList.remove('open', 'closing');
    document.body.classList.remove('reels-open', 'reels-as-tab');
    document.body.style.overflow = '';
    var header = document.querySelector('header');
    ...
    _reelsOpenedAsTab = false;
  }
  window._closeReelsTabSilently = _closeReelsTabSilently;
  ...
  function _updateReelsTitleBar(idx) {
    ...
    if (r && r._key) {
      if (window._fbDb && window._fbRef && window._fbIncrement && window._fbUpdate) {
        ...
        window._fbUpdate(window._fbRef(window._fbDb, _reelsViewsPath + '/' + r._key), _vObj);
      }
    }
  }
  ...
  function closeReelsModal(fromPopstate, fromReelsAll) {
    ...
    closeModalWithAnim(modal, 280, function() {
      ...
      // Reels মূল ট্যাব হিসেবে খোলা হলে — বন্ধ হবার পর আগের active tab restore করো
      if (_reelsOpenedAsTab) {
        _reelsOpenedAsTab = false;
        document.querySelectorAll('.tab-btn, .drn-item').forEach(function(b) { b.classList.remove('active'); });
        var capTab = (currentTab || 'watch').charAt(0).toUpperCase() + (currentTab || 'watch').slice(1);
        var mBtn = document.getElementById('tab' + capTab);
        var dBtn = document.getElementById('dsbTab' + capTab);
    ...
    try {
      ...
      var displayHash = tab === 'podcast' ? 'listen' : tab === 'watch' ? 'home' : tab === 'explore' ? 'explore' : tab;
  ...
  var _liveChannels = [];
  var _cachedLiveTvHtml = '';
  var _cachedStationsHtml = '';
  function _renderCombinedLiveTvRow() {
    var section = document.getElementById('liveTvSection');
    var scroll  = document.getElementById('liveTvScroll');
    if (!section || !scroll) return;
    var hasContent = !!(_cachedLiveTvHtml || _cachedStationsHtml);
    section.style.display = hasContent ? '' : 'none';
    if (!hasContent) return;
    var divider = (_cachedLiveTvHtml && _cachedStationsHtml) ? '<div class="tv-row-divider"></div>' : '';
    scroll.innerHTML = _cachedLiveTvHtml + divider + _cachedStationsHtml;
  }
  ...
  // ── YouTube URL → embeddable player detection (sports player) ──
  function _getYouTubeEmbed(url) {
    if (!url || !/youtu\.?be/i.test(url)) return null;
    // already an embed link
    var embedMatch = url.match(/youtube\.com\/embed\/([a-zA-Z0-9_-]{11})/i);
    if (embedMatch) return { embedUrl: 'https://www.youtube.com/embed/' + embedMatch[1], videoId: embedMatch[1] };
    // channel live link e.g. youtube.com/channel/UCxxxx/live (no fixed video id)
    var chMatch = url.match(/youtube\.com\/channel\/([a-zA-Z0-9_-]+)\/live/i);
    if (chMatch) return { embedUrl: 'https://www.youtube.com/embed/live_stream?channel=' + chMatch[1], videoId: null };
    // standard watch / youtu.be / shorts / live/VIDEOID links
    var idMatch = url.match(/(?:youtube\.com\/watch\?(?:.*&)?v=|youtu\.be\/|youtube\.com\/shorts\/|youtube\.com\/live\/)([a-zA-Z0-9_-]{11})/i);
    if (idMatch) return { embedUrl: 'https://www.youtube.com/embed/' + idMatch[1], videoId: idMatch[1] };
    return null;
  }
  ...
  function openSportsStream(key) {
    ...
    function _loadSportsUrl(srcUrl, wrap, loop) {
      ...
      wrap.innerHTML = '';
      var _yt = _getYouTubeEmbed(srcUrl);
      if (_yt) {
        var ytParams = 'autoplay=1&playsinline=1&rel=0&modestbranding=1&iv_load_policy=3'
          + (loop && _yt.videoId ? '&loop=1&playlist=' + _yt.videoId : '');
        var ytSep = _yt.embedUrl.indexOf('?') > -1 ? '&' : '?';
        wrap.innerHTML = '<iframe src="' + _yt.embedUrl + ytSep + ytParams + '" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen style="width:100%;height:100%;display:block;"></iframe>';
      } else if (srcUrl.match(/\.m3u8/i) || srcUrl.includes('m3u8')) {
  ...
  function renderLiveTvSection() {
    ...
    _cachedLiveTvHtml = sportsHtml + tvHtml;
    _renderCombinedLiveTvRow();
    _startSportsCountdowns();
  }
  ...
  window.addEventListener('popstate', function(e) {
    ...
    // (reels-as-tab হলে এটা স্কিপ করো, normal tab popstate flow এ যাবে)
    var _reelsModal = document.getElementById('reelsModal');
    if (_reelsModal && _reelsModal.classList.contains('open') && !document.body.classList.contains('reels-as-tab')) {
      closeReelsModal(true);
      return;
    }
    // Reels এর stale hash entry — modal বন্ধ, শুধু skip করো
    if (e.state && e.state.modal === 'reels' && !document.body.classList.contains('reels-as-tab')) return;
  ...
  (function() {
    ...
    if (hash.startsWith('v_') || hash.startsWith('cat_') || hash.startsWith('c_')) {
      ...
      window._pendingHash = hash;
      // Listen tab নেভিগেশন থেকে hidden — episode/station mini player-এ play হবে, watch tab-ই underlying tab থাকবে
      var targetTab = 'watch';
    ...
    } else if (hash && !['watch','home','fm','podcast','listen','reels'].includes(hash)) {
      // unknown hash — watch tab এ যাও
      window._pendingHash = hash;
      switchTab('watch', true);
      safeHistoryReplace({ type: 'tab', tab: 'watch' }, '', '#' + hash);
    } else {
      // Listen tab hidden — fm/listen hash এলেও watch tab-এ থাকো (podcast playback/modal স্বাভাবিকভাবেই কাজ করবে)
      // Watch tab এখন "Home" নামে, hash #home (পুরনো #watch link-ও backward-compat হিসেবে কাজ করবে)
      var tabMap = { 'fm': 'watch', 'listen': 'watch', 'watch': 'watch', 'home': 'watch', 'explore': 'explore', 'reels': 'reels' };
      var tab = tabMap[hash] || 'watch';
      var displayHash = tab === 'watch' ? 'home' : tab;
      switchTab(tab, true);
  ...
  function renderPodcastCategories() {
    ...
    var cards = cats.map(function(cat) {
      var thumbHtml = cat.img
        ...
        : '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#2e1b1a,#3e1916);font-size:1.4rem;">🎙️</div>';
  ...
  function pepBuildRow(ep, i, onclick, titleOnly) {
    ...
    if (!titleOnly) {
      ...
      metaHtml = metaParts.length ? '<div class="pep-ep-meta">' + metaParts.join(' &middot; ') + '</div>' : '';
    }
      ...
      overlay.innerHTML =
        '<div style="width:100%;max-width:480px;background:var(--bg,#fff);border-radius:20px 20px 0 0;padding:24px 20px 36px;box-shadow:0 -4px 32px rgba(0,0,0,0.18);">' +
          ...
          '<div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;">' +
            '<div style="width:44px;height:44px;border-radius:12px;background:var(--accent-light,#fee9e8);display:flex;align-items:center;justify-content:center;">' +
              '<svg viewBox="0 0 24 24" fill="none" stroke="var(--accent,#EF4444)" stroke-width="2" width="22" height="22"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-4.95"/></svg>' +
            '</div>' +
            '<div>' +
          ...
          '<div style="display:flex;gap:10px;">' +
            ...
            '<button onclick="_doAppUpdate()" style="flex:2;padding:12px;border-radius:12px;border:none;background:var(--accent,#EF4444);color:#fff;font-family:\'DM Sans\',sans-serif;font-size:0.9rem;font-weight:700;cursor:pointer;">আপডেট করুন</button>' +
          '</div>' +
        '</div>';
  ...
  function renderSidebarCats() {
    ...
    if (podCats.length) {
      aCont.innerHTML = podCats.slice(0, 9).map(function(cat) {
        ...
        return '<div class="sb-cat-item" onclick="closeSidebar();setTimeout(function(){switchTab(&quot;watch&quot;);setTimeout(function(){openPodcastEpModal(&quot;' + cat._key + '&quot;)},200)},200)">'
          + thumbHtml
          + '<div class="sb-cat-info">'
          + '<div class="sb-cat-name">' + (cat.title || cat._key) + '</div>'
          + '<div class="sb-cat-sub">' + sub + '</div>'
...
(function() {
  ...
  window.loadExploreNews = function(force) {
    ...
    var skeletonItem = '<div class="explore-news-skel-item"><div class="explore-news-skel-thumb"></div><div class="explore-news-skel-body"><div class="skel-line skel-line--full"></div><div class="skel-line skel-line--med"></div><div class="skel-line skel-line--short"></div></div></div>';
    ...
    if (_newsCache.allItems.length) {
      renderExploreNews(_newsCache.allItems.slice(0, 9));
      _newsLoaded = true;
      stopSpin();
      return;
    }
    ...
    fetch(proxyUrl)
      ...
      .then(function(xml) {
        ...
        _newsCache.allItems = all;
        renderExploreNews(all.slice(0, 9));
        _newsLoaded = true;
        stopSpin();
      })
      .catch(function() {
  ...
  function _exploreNewsCardHtml(item, i) {
    ...
    return '<a class="explore-news-item" style="animation-delay:' + (i * 0.07) + 's" href="' + link + '" target="_blank" rel="noopener">' +
      '<div class="explore-news-cover">' +
        '<img class="explore-news-thumb" src="' + thumbSrc + '" alt="" loading="lazy" ' + onerrorAttr + '>' +
        '<div class="explore-news-source-badge">' +
          '<img class="explore-news-source-icon" src="' + FALLBACK + '" alt="">' +
          '<span class="explore-news-source-name">প্রথম আলো</span>' +
        '</div>' +
      '</div>' +
      '<div class="explore-news-body">' +
        (pubDate ? '<div class="explore-news-meta"><span class="explore-news-time">' + formatNewsDate(pubDate) + '</span></div>' : '') +
  ...
  function renderExploreNews(items) {
    var list = document.getElementById('exploreNewsList');
    if (!list) return;
    list.innerHTML = items.map(function(item, i) { return _exploreNewsCardHtml(item, i); }).join('');
  }
